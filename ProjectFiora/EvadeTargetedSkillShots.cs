...
namespace FioraProject
{
    ...
    internal class EvadeTarget
    {
        ...
        internal static void Init()
        {
            ...
            var evadeMenu = new Menu("Evade mục tiêu có skill định hướng", "Né mục tiêu");
            {
                evadeMenu.Bool("W", "Dùng W");
                var aaMenu = new Menu("Tự động tấn công", "AA");
                {
                    aaMenu.Bool("B", "Tấn công cơ bản", false);
                    aaMenu.Slider("BHpU", "-> nếu Hp < (%)", 35);
                    aaMenu.Bool("C", "Tấn công crit", false);
                    aaMenu.Slider("CHpU", "-> nếu Hp < (%)", 40);
